...
export const gerarAdminAuthToken = async () => {
    ...
    if (response.status !== 200 || !response.body.token || !response.body.token.token) {
        console.error('Falha ao fazer login de ADMIN para obter token. Resposta:', response.body);
        throw new Error('Não foi possível obter o token de autenticação de ADMIN. Verifique as credenciais do usuário administrador e a rota de login.');
    }
