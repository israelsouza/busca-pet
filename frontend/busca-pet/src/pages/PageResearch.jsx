...
import {
  getPublicacoesPorTexto,
  getPetsPorArea,
} from "../assets/services/api/posts.js";
...
function PageResearch() {
  ...
  const handleGoToMap = useCallback(async () => {
    ...
    try {
      ...
      setMapError(
        "Não foi possível carregar o mapa para esta região. Tente novamente. Verifique se o termo de busca é um endereço válido."
      );
    ...
    <div className={styles.main}>
      ...
      {selectedPublication ? (
        <div className={styles.publicationDetailContainer}>
          <Buttonposts
            ...
            textoPrimeiroCategoria={
              selectedPublication.POS_TIPO == "Perdido"
        ...
        <div className={styles.map_view}>
          ...
          {loadingMap && (
            <p className={styles.loadingMessage}>Carregando mapa e pets...</p>
          )}
        ...
        <div className={styles.search_container}>
          ...
          {loadingSuggestions && (
            <p className={styles.loadingMessage}>Carregando sugestões...</p>
          )}
          {errorSuggestions && (
            <p className={styles.errorMessage}>{errorSuggestions}</p>
          )}
          ...
          {suggestions.length > 0 && (
            <div className={styles.suggestions_box}>
              <ul className={styles.suggestions_list}>
                {suggestions.map((pub) => (
                  <div key={pub.POS_ID} className={styles.suggestion_item}>
                    <div className={styles.suggestion_itemFoto}>
                      <strong>
                        <img
                          src={`data:image/jpeg;base64,${pub.USU_FOTO}`}
                          alt=""
                    ...
                    <div className={styles.suggestion_itemInfo}>
                      <div>
                        <strong>
                          {" "}
                          {pub.PES_NOME
                            ? pub.PES_NOME.charAt(0).toUpperCase() +
                              pub.PES_NOME.slice(1).toLowerCase()
                            : "Foto user"}{" "}
                        </strong>
                      </div>
                      <div>
                ...
                <p
                  className={styles.suggestions__linkMap}
